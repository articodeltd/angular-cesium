...
export class BasicStaticPrimitiveDesc extends BasicDesc implements OnInit {
  ...
  draw(context, id, entity: AcEntity): any {
    ...
    if (!this._cesiumObjectsMap.has(id)) {
      ...
      const updatedPrimitive = this._staticPrimitiveDrawer.update(primitive, geometryProps, instanceProps, primitiveProps);
      this._cesiumObjectsMap.set(id, updatedPrimitive);
    }
  }
}
