...
export class PerformanceFormComponent implements OnInit {
  ...
  private interval = 2;
  private sendOption = SendOption.ONE_BY_ONE;
  ...
  change(){
    this.http.post('http://localhost:3000/change',{interval: this.interval,
                             numOfEntities: this.numOfEntities,
                             sendOption: this.sendOption}).catch(this.handleError)
                             .subscribe(()=> {
                               this.cleanMap.emit();
                               alert('changed');
                             });
