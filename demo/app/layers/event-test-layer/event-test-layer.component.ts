...
export class EventTestLayerComponent implements OnInit {
    ...
    constructor(private eventSelectionManager: MapSelectionService,
    ...
    ngOnInit(): void {
        ...
        // Send mouse location
        this.eventSelectionManager.select({
    ...
    testPlonter() {
        ...
        this.eventSelectionManager.select({event: CesiumEvent.LEFT_CLICK, pick: PickOptions.PICK_FIRST})
    ...
    testPriority() {
        const o1 = this.eventSelectionManager.select({
            event: CesiumEvent.LEFT_CLICK,
            priority: 1,
            pick: PickOptions.PICK_FIRST
        });
        ...
        const o2 = this.eventSelectionManager.select({
            event: CesiumEvent.LEFT_CLICK,
            priority: 2,
            pick: PickOptions.PICK_FIRST
        });
        ...
        const o3 = this.eventSelectionManager.select({
            event: CesiumEvent.LEFT_CLICK,
            priority: 2,
            pick: PickOptions.PICK_FIRST
        });
        ...
        const o4 = this.eventSelectionManager.select({
            event: CesiumEvent.LEFT_CLICK,
            priority: 3,
            pick: PickOptions.PICK_FIRST
        });
    ...
    testColorChange() {
        ...
        this.eventSelectionManager.select(inputConf).map((result) => result.entities[0]).filter((entity) => entity.id === 0).subscribe((entity) => {
